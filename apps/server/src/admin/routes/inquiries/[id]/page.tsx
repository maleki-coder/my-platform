...
export default function InquiryDetailsPage() {
  ...
  if (isLoading) {
    return (
      <Container className="p-6 flex items-center justify-center min-h-[200px]">
    ...
    <div className="flex flex-col gap-y-4">
      ...
      <Container className="p-6 flex flex-col gap-y-6">
        <div className="flex justify-between items-start border-b border-ui-border-base pb-4">
          <div>
            <Heading level="h1" className="text-ui-fg-base">جزئیات استعلام</Heading>
            <Text className="text-ui-fg-subtle font-mono text-xs mt-2 uppercase">
              شناسه: {inquiry.id}
            </Text>
          </div>
          <Badge 
            color={inquiry.status === "completed" ? "green" : inquiry.status === "reviewed" ? "blue" : "orange"} 
            size="large"
          >
            {inquiry.status === "completed" ? "تکمیل شده" : inquiry.status === "reviewed" ? "بررسی شده" : "در انتظار بررسی"}
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex flex-col gap-y-1">
            <Text className="text-ui-fg-subtle text-xs font-medium">نام مشتری</Text>
            <Text className="text-ui-fg-base">{inquiry.customer_name}</Text>
          </div>
          <div className="flex flex-col gap-y-1">
            <Text className="text-ui-fg-subtle text-xs font-medium">ایمیل ارتباطی</Text>
            <Text className="text-ui-fg-base" dir="ltr">{inquiry.email}</Text>
          </div>
          <div className="flex flex-col gap-y-1">
            <Text className="text-ui-fg-subtle text-xs font-medium">شماره تماس</Text>
          ...
          <div className="col-span-1 md:col-span-3 flex flex-col gap-y-2 mt-2 border-t border-ui-border-base pt-4">
            <Text className="text-ui-fg-subtle text-xs font-medium">یادداشت مشتری</Text>
      ...
      <Container className="p-0 overflow-hidden mt-2">
        ...
        <div className="w-full overflow-x-auto relative">
          <Table>
            <Table.Header>
              <Table.Row>
                <Table.HeaderCell className="sticky left-0 bg-ui-bg-subtle border-ui-border-base px-6 whitespace-nowrap">
                ...
                <Table.HeaderCell className="px-6 whitespace-nowrap">
                  دیتاشیت (Datasheet)
                </Table.HeaderCell>
              </Table.Row>
            </Table.Header>
              ...
              {inquiry.items?.length === 0 ? (
                ...
                inquiry.items?.map((item, index) => (
                  <Table.Row key={item.id || index}>
                    <Table.Cell className="sticky left-0 bg-ui-bg-base border-ui-border-base font-bold text-ui-fg-base px-6 shadow-[1px_0_0_0_var(--borders-base)] whitespace-nowrap" dir="ltr">
                      {item.part_number}
                    </Table.Cell>
                    <Table.Cell className="px-6 text-center font-medium">
                      {item.quantity}
                    </Table.Cell>
                    <Table.Cell className="px-6 text-center text-ui-fg-subtle font-mono" dir="ltr">
                      {item.target_price ? `$${item.target_price}` : "نامشخص"}
                    </Table.Cell>
                    <Table.Cell className="px-6 whitespace-nowrap">
                      {item.datasheet_url ? (
                        ...
                        <Text className="text-ui-fg-subtle text-sm">بدون فایل</Text>
                      )}
                    </Table.Cell>
                  </Table.Row>
                ))
