...
export const authenticateWithPhone = async (phone: string) => {
  try {
    ...
    // CASE 1: SDK returned an error as a raw string
    if (typeof response === "string") {
      throw new Error(response)
    }

    // CASE 2: SDK returned an object but missing required fields
    if (!response.location) {
      throw new Error("Invalid response from server")
    }

    // CASE 3: location but not OTP → still an error
    if (response.location !== "otp") {
      throw new Error("Unexpected login step")
    }
    return true
  } catch (err: any) {
